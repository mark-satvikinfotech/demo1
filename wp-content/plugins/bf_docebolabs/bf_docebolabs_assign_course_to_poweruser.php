...
if(isset($_REQUEST['DoceboIdEmail']) && $_REQUEST['DoceboIdEmail'] != '' && $_REQUEST['DoceboIdEmail'] != null){
	...
	if(isset($courseId) && $courseId != null && $courseId != ''){
		...
		if($debug == true){
			$fp = fopen('debug_'.basename(__FILE__).'.txt', 'a');			fwrite($fp, "Assign course result:".print_r($result1,true));
			fwrite($fp, "docebo api attempted assigning course id ".$courseId." to power user id ".$poweruserid.": ".date('d-m-Y H:i:s')." - ".print_r($result, true)."\r\n");
			fclose($fp);
		}
	}
}
