<?php
function manage_geo_to_ifs()
{
	include_once('lib/infusionsoft/is_config.php');
	//$option_exists = get_option("geopal_credential");
	...
	if($cred_arr['uname'] == "hes0012" &&  $cred_arr['pass'] == "48hrlaunch" && $cred_arr['app_name'] == "qn241" && $cred_arr['app_key'] == "c31ce6f980e4c9de594a39bb78fda91d" )
	{
	  ...
	  echo $crt_tag = $profile_arr['apptmnt_crt_tag'];
	  $upd_tag = $profile_arr['apptmnt_upd_tag'];
	  $del_tag = $profile_arr['apptmnt_del_tag'];	
		
	  echo "<pre>";
	  $a = get_option('Geopal_log_generate');
	  $str = unserialize($a);	
	  echo $tag =$str['tag'];
	 ...
	 /* $returnFields = array('Id','_jobid0','Email');
						...
						print_r($con);
						exit;*/
	  $curlSession = curl_init('https://app.geopalsolutions.com/api/jobs/get?job_id=17742284');
				 ...
				 $job_emp_fname = $get_jb_data->employee->first_name;
				 $job_emp_lname = $get_jb_data->employee->last_name;
				 ...
				 {
					echo $st_id =$value->job_status_id;
					if($st_id == $job_status_id)
					{
					  $change_st_msgid = $value->id;
					  echo $change_st_msg = $msg_status_arr[$job_status_id];
	    ...
	    if($tag == "job")
			  {
					...
					$job_per_email = $jb_data->job->person->email;
					$job_per_created_on = $jb_data->job->person->created_on;
					$job_per_updated_on = $jb_data->job->person->updated_on;
					...
					$jobdata = array('_jobid0' => "$job_id",
						...
						else
						{
							//echo "update";
							$contactId = $con[0]['Id'];
							$contactId = $app->updateCon($contactId, $jobdata);
			  ...
			  else if($tag == "job_workflow_file")
			  {
				 ...
				 if(isset($str['job_workflow']))
				 {
					...
					/* $job_work_file_dt = array(
					   '_jobid0' => "$jb_wrkflow_fl_job_id",
					   '_Appointmentworkflowfileid' => "$jb_wrkflow_fl_id",
					   '_Appointmentworkflowid' => "$jb_wrkflow_fl_wrkflw_id",
					   '_Appointmentcompanyid' => "$jb_cmp_id",
					   '_Appointmentcompanyname' => "$jb_cmp_name",
					   '_Appointments3fileid' => "$jb_wrkflow_fl_s3file_id",
					   '_Appointments3filethumbnailid' => "$jb_wrkflow_fl_s3file_thumbnail_id",
					   '_link2file' => "$jb_wrkflow_fl_link_2_file",
					   '_Appointmentworkflowfilecreatedon' => "$jb_wrkflow_fl_created_on",
				 ...
				 $jobdata = array('_jobid0' => "$job_id",
						...
						$notes = array(
						         'ContactId' => $con[0]['Id'],
								 'CreationNotes' => $str['job_workflow_file'],
								 'ObjectType' => "Note",
								 'CreationDate' => "$job_created_on"
						);
			  ...
			  { 
                 ...
                 /*$job_workflow = array(
							'_jobid0'=> "$jb_wrkflow_job_id",
							'_Appointmentworkflowid' => "$jb_wrkflow_id",
							'_Appointmentcompanyid' => "$jb_wrkflow_cmp_id",
							'_Appointmentcompanyname'=> "$jb_wrkflow_cmp_name",
							'_Appointmenttemplateworkflowid' => "$jb_wrkflow_template_workflow_id",
							'_Appointmentworkflowcreatedon' => "$jb_wrkflow_created_on",
							'_Appointmentworkflowupdatedon' => "$jb_wrkflow_updated_on"
				 );*/
			  ...
			  else if($tag = "job_field")
			  {
						...
						curl_setopt(
							...
							$responseAsAnArray = (is_null($response) || ($response === false))? array(): json_decode($response, true);
							$get_jb_data = json_decode($response);
							//$jb_data = print_r($get_jb_data->job);
							//echo $get_jb_data->job->id;
	...
	{
		echo "<h4>Your provided credential don't match. Try with correct credentail.</h4>";
	}
}
