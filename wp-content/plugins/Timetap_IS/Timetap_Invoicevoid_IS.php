<?php
 ...
 include('infusion_config.php');
  $config = json_decode(file_get_contents($timetap_invoicevoid_log_file_name),true);
  ...
  {
		$timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,"\n\n"."****************** Invoice Cron Start*******"."\n");
		$date = date("Y-m-d H:i:s");
		$timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,'Date: '.$date."\n");
		$timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,'Total Invoice: '.$total_invoice."\n");
		$timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,'Last updated Invoice: '.$last_updated_invoice."\n");
		$last_total_contacts = $config['total']; 
		foreach($invoices as $key => $value)		{			$Invoice_Id = $value->invoiceId;
		    $Client = $value->client;
		    if(empty($value->invoiceItem))
			{
				$timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,'Invoice item not found:'.$Invoice_Id."\n");
			}
			if($Client->emailAddress != "" && !empty($value->invoiceItem))			{	
				...
				if($value->invoiceId != "")				{
				   $timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,'Invoice ID: '.$Invoice_Id."\n");
				}
				if(isset($Client) && !empty($Client)) 				{
				  $clientId =  $Client->clientId;
				  $contactData = array();
				  if($clientId != '')				  {
				    $contactData['_ClientId'] = "$clientId";
				  }										$apptmnt_data = $timetap->get_client_appointment($sessiontok,$clientId);
					$apptdata = json_decode($apptmnt_data);
					$app_prev= array();
					...
					if(isset($prevalues) && $prevalues != '')					{ 
					  $pre_dt = date('Y-m-d', (int)$prevalues);
					  $pre_time = date('Hi', (int)$prevalues); 
					  ...
					  foreach($aptm_pr_dt as $key=>$val)
					  {
						if($pre_time == $val->startTime)
						{
							...
							{
						      $contactData['_LastAppointmentLocation'] = $val->location->locationName;
							}
			     ...
			     $conID = $infusionsoft->contacts('xml')->addWithDupCheck($contactData,  'Email');
				  ...
				  $timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME,'Contact Id: '.$conID."\n");
				if($conID != '')
				{
			        ...
			        $timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME, "Order Id : ".$order_id." \n\n");
					$total_price = 0;	
		    ...
		    $timetap->update_LogFile("last_updated", $data_value,$timetap_invoicevoid_log_file_name);
        }
        $config = json_decode(file_get_contents($timetap_invoicevoid_log_file_name),true);		
		$data_value = $config['current_page']+ 1;
		$timetap->update_LogFile("current_page", $data_value, $timetap_invoicevoid_log_file_name);
		$current_total = $config['current_page'] * $config['limit'];
		if($current_total <= $total_invoice)		{
			$timetap->update_LogFile("total", $total_invoice, $timetap_invoicevoid_log_file_name);
		}
   }
   $timetap->writeToLog(TIMETAP_INVOICEVOID_CRON_FILE_NAME, "****************Invoice Cron end********************* \n\n");
   echo "Success";
  exit;
  ?>